...
;; coerce the arguments to each other's type even if they already have the
;; same type. Therefore, he reasons, we need to put procedures in the
;; coercion table to "coerce" arguments of each type to their own type. For
;; example, in addition to the scheme-number->complex coercion shown above,
;; he would do:
...
;; In that situation we would enter infinite loop, where apply-generic is
;; being called over and over again with the same arguments and their types
...
;; Louis is wrong, since after coercing arg types to themselves nothing changes.
      ...
      (if proc
          ...
          (if (= (length args) 2)
              (let ((type1 (car type-tags))
                    ...
                    (a2 (cadr args)))
                (if (eq? type1 type2)
                    (error "No method for this type" (list type1))
                    (let ((t1->t2 (get-coercion type1 type2))
                          (t2->t1 (get-coercion type2 type1)))
                      (cond (t1->t2
                             (apply-generic op (t1->t2 a1) a2))
